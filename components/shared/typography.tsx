...
type Variant = 'h1' | 'h2' | 'h3' | 'h4' | 'p' | 'notes'
...
export default function Typography({variant, className, children}: TypographyProps) {
    switch (variant) {
        case 'h1': return <h1 className={cn('text-2xl font-bold', className)}>{children}</h1>
        ...
        case 'p': return <p className={cn('text-sm', className)}>{children}</p>
        case 'notes': return <p className={cn('text-xs', className)}>{children}</p>
    }
}